...
public class LocalNameMapper {
	...
	public static void addClassToFreebaseDbpediaInstanceMapping(String freebaseInstance, String dbpediaInstance, 
			String instanceMapping, String output) throws Exception {
		HashMap<String, ArrayList<String>> dbpediaClassMap = loadClassMap(dbpediaInstance);
		System.out.println("dbpedia instance-class info loaded");
		HashMap<String, ArrayList<String>> freebaseDbpediaInstanceMap = loadInstanceMap(instanceMapping);
		...
		for (String line = br.readLine(); line != null; line = br.readLine()) {
			String[] part = line.split(" ");
			if (freebaseDbpediaInstanceMap.containsKey(part[0])) {
				ArrayList<String> dbpediaInst = freebaseDbpediaInstanceMap.get(part[0]);
				for (String s : dbpediaInst)
					pw.println("localname\t" + part[0] + "\t" + part[2] + "\t" + s + "\t" + dbpediaClassMap.get(s));
			}
			lineCount++;
