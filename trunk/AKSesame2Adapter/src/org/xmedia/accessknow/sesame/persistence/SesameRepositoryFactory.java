...
public class SesameRepositoryFactory {
	...
	private enum RepositoryType {
		...
		RDFS_MEMORY_PERSISTENT(SesameRepositoryFactory.RDFS_MEMORY_PERSISTENT), RDF_MEMORY_VOLATILE(
				SesameRepositoryFactory.RDF_MEMORY_VOLATILE), RDF_NATIVE(
				SesameRepositoryFactory.RDF_NATIVE), RDFS_NATIVE(
				SesameRepositoryFactory.RDFS_NATIVE);
		...
		private void map(String urn, RepositoryType op) {
			if (tokenMap == null)
				tokenMap = new HashMap<String, RepositoryType>();
			tokenMap.put(urn, op);
		}
	...
	protected SesameRepositoryFactory(String sesameRootPath)
			throws RepositoryException {

		this.sesameRootPath = (sesameRootPath.endsWith(File.separator) ? sesameRootPath
				+ SESAME_REPO_DIR
	...
	private void reloadRepositoryManager() throws RepositoryException {
		if (theManager != null) {
			try {
				if (theManager.getAllRepositories().size() > 0)
					theManager.refresh();
	 ...
	 * 
	 */
	protected synchronized void delete(String ontologyUri)
	...
	protected synchronized void clearAll() throws IOException,
			RepositoryException {

		File repositoriesDir = new File(sesameRootPath + "\\"
	...
	public synchronized Repository loadRemoteRepository(URI ontologyUri,
	...
	public synchronized Repository loadRepository(URI theOntologyUri)
			throws RepositoryException, RepositoryConfigException {
		...
		RepositoryConfig config = RepositoryConfigUtil.getRepositoryConfig(
				theManager.getSystemRepository(), repositoryId);
	...
	private Repository loadRepository(RepositoryConfig config,
			String repositoryId) throws RepositoryConfigException,
			RepositoryException {

		Repository theRepository = createNonInitializedRepository(config
	 ...
	 * @throws RepositoryConfigException
	 */
	private Repository createNonInitializedRepository(
			RepositoryImplConfig config) throws RepositoryConfigException {
		...
		if (config instanceof DelegatingRepositoryImplConfig) {
			RepositoryImplConfig delegateConfig = ((DelegatingRepositoryImplConfig) config)
					.getDelegate();
	...
	private RepositoryConfig putRepositoryConfig(Repository system,
			URI theOntologyUri, RepositoryType repoType) throws Exception {
		...
		case RDFS_MEMORY_PERSISTENT:
			config = RdfsMemoryRepositoryFactory.createRepository(system,
					repositoryId);
			break;
		case RDF_NATIVE:
			config = RdfNativeRepositoryFactory.createRepository(system,
					repositoryId);
			break;
		case RDFS_NATIVE:
			config = RdfsNativeRepositoryFactory.createRepository(system,
					repositoryId);
			break;
		default:
	 ...
	 * @param type
	 * @param remoteType
	 *            must be not null only if itsType is
	 *            SesameRepositoryFactory.REMOTE
	 * @return
	 * @throws Exception
	 */
	public synchronized Repository createRepository(URI ontologyUri, URI type,
		...
		if (repositoryType == null)
			...
			if (remoteHandle != null) {
				putRepositoryConfig(remoteHandle.getRemoteSystem(),
						remoteHandle.getOntologyUri(), repositoryType);
				...
				if (repositoryType.equals(RepositoryType.RDF_MEMORY_VOLATILE))
					...
					repository = loadRepository(
							putRepositoryConfig(getSystemRepository(),
									ontologyUri, repositoryType),
	...
	public synchronized Repository createRepositoryWithIndex(URI ontologyUri,
			URI type, String indices) throws Exception {
		RepositoryType repositoryType = RepositoryType.forUrn(type.toString());
		String repositoryId = fsTransduceUri(ontologyUri);

		if (repositoryType != RepositoryType.RDFS_NATIVE
				&& repositoryType != RepositoryType.RDF_NATIVE)
			throw new Exception("Unsopported repository type '" + type
					+ "'. Only native store repositories may use indices.");
		...
		theRepository.setDataDir(theManager
				.getRepositoryDir(fsTransduceUri(ontologyUri)));
		theRepository.initialize();

		RepositoryConfigUtil.updateRepositoryConfigs(getSystemRepository(),
				new RepositoryConfig(repositoryId, new SailRepositoryConfig(
						config)));
	...
	//		
	// SailRepository theRepository = new SailRepository(sail);
	// theRepository.setDataDir(theManager.getRepositoryDir(fsTransduceUri(
	// ontologyUri)));
	// theRepository.initialize();
	//		
	// return theRepository;
