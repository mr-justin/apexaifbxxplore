...
public class ForServlet {
	...
	private GeneralCategory createGeneralCategory(Node node) {
		...
		if (n!=null && !n.getNodeValue().equals("") ) {
			cat.addComponentCategory(schemaFactory.createEnumerationCategory().addInstanceElement(schemaFactory.createInstance(n.getNodeValue())));
			return cat;
		}
...
//			System.out.println("name="+node.getNodeName());
	...
	private String result2XML(XFacetedResultSet resultSet, Document doc2, long queryTime) {
        try {        	         
        } catch (Exception ex) {
            System.out.println(ex);
        }		
        try {
        	...
        	if (map.getNamedItem("type").getNodeValue().equals("all") || map.getNamedItem("type").getNodeValue().equals("instanceList") ) {
                ...
                for (int i=start; i<end; i++) {
                	...
                	subitem.setAttribute("name", java.net.URLDecoder.decode(result.getLabel()));
                	subitem.setAttribute("uri", result.getURI());
//                	subitem.setAttribute("snippet", result.getTextDescription());
                	subitem.setAttribute("snippet",java.net.URLDecoder.decode(resultSet.getSnippet(i)));
	...
	public static void main(String[] args) {
		try {
			...
			String queryXML = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
					...
					"	<targetNode index=\"0\"/>" +
					"	<graph>" +
					"		<queryNode relation=\"\" instanceConstraint=\"&lt;http://dbpedia.org/resource/Ken_Hughes>\" conceptConstraint=\"&lt;http://dbpedia.org/class/yago/City108524735>\" keyword=\"earthquake\">" +
					"		</queryNode>" +
					"	</graph>" +
					"</query>";
