...
public class TermFactoryImpl implements TermFactory {
	...
	public Term createTermForInstances(GeneralCategory cat) {
		...
		if (cat instanceof AttributeKeywordCategory)
			return createTerm( new AttributeValue(((AttributeKeywordCategory)cat).getAttribute(), ((AttributeKeywordCategory)cat).getKeyword()).getCombination(), FieldType.ATTRIBUTE_VALUE);
