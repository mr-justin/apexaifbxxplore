...
public class SearchSessionService {
		...
		if (query instanceof QueryGraph) {
			...
			for (Facet f : nodeList) {
				if (f.equals(targetNode)) {
					CompoundCategory cc = SchemaFactoryImpl.getInstance()
							.createCompoundCategory(1); // AND
					if (f instanceof Concept) {
						Concept c = (Concept) f;
						cc.addComponentCategory(SchemaFactoryImpl.getInstance()
								.createCategory(c.getURI()));
					} else if (f instanceof Attribute) {
						Attribute a = (Attribute) f;
						cc.addComponentCategory(SchemaFactoryImpl.getInstance()
								.createAttributeKeywordCategory(a.getURI(),
										a.getLabel()));
					} else if (f instanceof Instance) {
						Instance i = (Instance) f;
						EnumerationCategory ec = SchemaFactoryImpl
								.getInstance().createEnumerationCategory()
								.addInstanceElement(
										SchemaFactoryImpl.getInstance()
												.createInstance(i.getURI()));
						cc.addComponentCategory(ec);
					} else if (f instanceof Litteral) {
						Litteral l = (Litteral) f;
						cc.addComponentCategory(SchemaFactoryImpl.getInstance()
								.createKeywordCategory(l.getLabel()));
					}
					graph.add(cc);
			...
			for (GraphEdge e : edgeList) {
				...
				graph.add(SchemaFactoryImpl.getInstance().createRelation(
						rel.getURI()), from, to);
			}
			...
			operationHistory
					.add(new QueryGraphOperation(new GraphImpl(), graph));
			LinkedList<XFacetedResultSetForMultiDataSources> resultHistory = new LinkedList<XFacetedResultSetForMultiDataSources>();
			...
			CompoundCategory cc = SchemaFactoryImpl.getInstance()
					.createCompoundCategory(1); // AND
			cc.addComponentCategory(SchemaFactoryImpl.getInstance()
					.createKeywordCategory(str));
			graph.add(cc); // 0
			graph.setTargetVariable(0);
			graph.setDataSource(0, (String) Config.readDSConfigFile(
					"config" + File.separatorChar + "datasrc.cfg").get(
					"defaultDataSet"));
	...
	public ResultPage getPage(int pageNum, int nbResultsPerPage)
			throws Exception {
		if (session == null
				|| session.get("resultHistory") == null)
			return null;
		LinkedList<XFacetedResultSetForMultiDataSources> resultHistory = (LinkedList<XFacetedResultSetForMultiDataSources>) session.get("resultHistory");
		XFacetedResultSetForMultiDataSources currentResult = resultHistory
				.getLast();
	...
	protected ResultPage transform(XFacetedResultSetForMultiDataSources xres,
		...
		try {
			...
			// add category facets
			com.ibm.semplore.search.Facet[] semploreFacets = xres
					.getCategoryFacets();
			for (int i = 0; i < semploreFacets.length; i++) {
				SchemaObjectInfo info = semploreFacets[i].getInfo();
				Facet facet = new Concept(info.getLabel(), info.getURI(),
						activeSource);
			...
			for (int i = 0; i < semploreFacets.length; i++) {
				SchemaObjectInfo info = semploreFacets[i].getInfo();
				Facet facet = new Relation(info.getLabel(), info.getURI(),
						activeSource);
				facet.setResultNb(semploreFacets[i].getCount());
				facetList.add(facet);
			}
			...
			// always make the active source the first element of the source
			// list
			...
			if (sourceFacets != null)
				for (Entry<String, Integer> entry : sourceFacets.entrySet()) {
					Source s = new Source(entry.getKey(), null, entry
							.getValue());
					sourceList.add(s);
				}
			...
			for (int i = start; i < end && i < xres.getLength(); i++) {
				...
				ResultItem item = new ResultItem(info.getURI(), xres
						.getScore(i), "text document", info.getLabel(), xres
						.getSnippet(i));
				resultItemList.add(item);
			}

			ResultPage page = new ResultPage(resultItemList, activeSource,
					pageNum);
			return page;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ResultPage refine(Query query, int nbResultsPerPage)
		...
		if (query instanceof QueryGraph) {
			...
			String ds = currentGraph.getDataSource(currentGraph
					.getTargetVariable());
			currentGraph = ko.applyTo(currentGraph);
			operationHistory.add(ko);
			session.put("operationHistory",
					operationHistory);
			session.put("currentGraph",
					currentGraph);
			...
			int id = SemplorePool.acquire();
			QueryEvaluator eval = SemplorePool.getEvaluator(id);
			XFacetedResultSetForMultiDataSources newResult = eval.evaluate(
					graph, helper);
			SemplorePool.release(id);

			ResultPage ret = transform(newResult, 1, nbResultsPerPage);
			resultHistory.add(newResult);
			session.put("resultHistory",
					resultHistory);
			return ret;
			...
			if (query instanceof Concept) {
				...
				Graph graph = new GraphImpl();
				graph.add(SchemaFactoryImpl.getInstance().createCategory(
						c.getURI())); // 0
				graph.setTargetVariable(0);
				graph.setDataSource(0, ds);
				HashMap<Integer, DocStream> helper = new HashMap<Integer, DocStream>();
				...
				Graph graph = new GraphImpl();
				graph.add(SchemaFactoryImpl.getInstance()
						.createUniversalCategory()); // 0
				graph.add(SchemaFactoryImpl.getInstance()
						.createUniversalCategory()); // 1
				graph.add(SchemaFactoryImpl.getInstance().createRelation(
						r.getURI()), 0, 1);
			...
			if (query instanceof ConceptSuggestion) {
				...
				ConceptSuggestionOperation cso = new ConceptSuggestionOperation(
						c);
				String currentDataSource = currentGraph
						.getDataSource(currentGraph.getTargetVariable());
				...
				if (c.getSource().getName().equals(currentDataSource)) {
					...
					int id = SemplorePool.acquire();
					QueryEvaluator eval = SemplorePool.getEvaluator(id);
					XFacetedResultSetForMultiDataSources newResult = eval
							.evaluate(graph, helper);
					SemplorePool.release(id);
				...
				RelationSuggestionOperation rso = new RelationSuggestionOperation(
						r);
				String currentDataSource = currentGraph
						.getDataSource(currentGraph.getTargetVariable());
	...
	public SeeAlso getSeeAlsoItem(String resultItemURL) {
		try {
			...
			int id = SemplorePool.acquire();
			QueryEvaluator eval = SemplorePool.getEvaluator(id);
			ArrayList<SchemaObjectInfoForMultiDataSources> array = eval
					.getSeeAlso(currentResult.getCurrentDataSource(),
	...
	private ArrayList<ResultItem> getResultList(
			XFacetedResultSetForMultiDataSources xres) {
		try {
			// set active source
			Source activeSource = new Source(xres.getCurrentDataSource(), null,
					xres.getLength());
			...
			for (int i = 0; i < semploreFacets.length; i++) {
				SchemaObjectInfo info = semploreFacets[i].getInfo();
				Facet facet = new Facet(info.getLabel(), info.getURI(),
						activeSource);
				facetList.add(facet);
			}
			// add relation facets
	...
	public ArraySnippet getArraySnippet(String resultItemURL) {
		try {
			...
			int id = SemplorePool.acquire();
			QueryEvaluator eval = SemplorePool.getEvaluator(id);
			String snippet_str = eval.getArraySnippet(currentResult
					.getCurrentDataSource(), currentResult.getDocID(index),
					resultItemURL);
			SemplorePool.release(id);
	...
	private ArraySnippet getSnippet(String resultItemURL, String snippet_str) {
		...
		if (snippet_str != null) {
			...
			while (tok.hasMoreTokens()) {
				...
				for (int i = 1; i <= 2; i++) {
					if ((i == 1 && type == Util4NT.CATEGORY)
							|| (i == 2 && type == Util4NT.ATTRIBUTE))
						continue;
					if (processed[i].startsWith("<"))
						processed[i] = processed[i].substring(1);
					if (processed[i].endsWith(">"))
						processed[i] = processed[i].substring(0, processed[i]
								.length() - 1);
				...
				if (type == Util4NT.CATEGORY) {
					...
					rel
							.add(new Couple(new Relation(labels[1],
									processed[1], null), new Instance(
									labels[2], processed[2], null)));
				} else if (type == Util4NT.ATTRIBUTE) {
					attr.add(new Couple(new Attribute(labels[1], processed[1],
							null), new Litteral(processed[2], null, null)));
				}
			}
		}
