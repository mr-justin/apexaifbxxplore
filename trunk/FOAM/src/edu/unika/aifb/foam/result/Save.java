...
public class Save {
		...
		while (iter.hasNext()) {
			...
			if (object2!=null) {
				...
				for (int j=0; j<rules.total(); j++) {
					combination.setValue(j,rules.process(object1,object2,j,structure));	//	individual feature/similarities are processed
				}
				resultList.set(object1,object2,value,combination.getAddInfo());	//	results are added to the similarity list
				resultList.set(object2,object1,value,combination.getAddInfo());	//	in both directions
			}
		}
		resultListLatest = resultList;
		saveCompleteEval(structure,resultListLatest,evalFile,cutoff,fileName);	//save mappings
	}
	...
	public static void saveCompleteEval(Structure structure, ResultList list, String evalFile, double cutoff, String fileName) {
		...
		UserInterface.print("Saved "+fileName+"\n");
/*		Evaluation evaluation = new Evaluation(structure,list,cutoff,evalFile);
		double[] eval = evaluation.returnEvaluation();
		out.write("\nPoint of cutoff:  precision "+eval[0]+"  recall "+eval[1]+"  f-measure "+eval[2]+" overlap "+eval[3]);
		out.write("\nPoint of maximum precision:  precision "+eval[4]+"  recall "+eval[5]+"  f-measure "+eval[6]+" overlap "+eval[7]);
		out.write("\nPoint of maximum f-measure:  precision "+eval[8]+"  recall "+eval[9]+"  f-measure "+eval[10]+" overlap "+eval[11]);
		out.write("\nPoint of maximum overlap:  precision "+eval[12]+"  recall "+eval[13]+"  f-measure "+eval[14]+" overlap "+eval[15]);
		out.write("\nPoint of maximum recall:  precision "+eval[16]+"  recall "+eval[17]+"  f-measure "+eval[18]+" overlap "+eval[19]"+\n");*/		
		out.close();
		} catch (Exception e) {
			UserInterface.print(e.getMessage());		
		}
