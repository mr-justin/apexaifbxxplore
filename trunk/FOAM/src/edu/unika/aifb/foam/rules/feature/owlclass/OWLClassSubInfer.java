 ...
 *
 */
public class OWLClassSubInfer implements Feature {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (next instanceof OWLClass) {
					entities.add(next);
					Set descriptions2 = next.getSubDescriptions(ontology.ontology);
					Iterator iter2 = descriptions2.iterator();
					while (iter2.hasNext()) {
						Description next2 = (Description) iter2.next();
						if (next2 instanceof OWLClass) {
							entities.add(next2);
							Set descriptions3 = next.getSubDescriptions(ontology.ontology);
