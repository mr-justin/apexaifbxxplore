...
public class ModelDefinitionContentProvider implements ITreeContentProvider {
	...
	public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
		...
		if (oldInput != newInput) {
			if ((newInput != null) && (newInput instanceof ModelDefinition)) {
				...
				m_modelDefinitionNode = new ModelDefinitionNode(this,
						m_modeldefinition);
	...
	public boolean openDialog(Collection<Collection<OWLPredicate>> queries) {
		...
		for (int j = 0; j < n; j++) {
			...
			for (OWLPredicate predicate : query) {
				...
				if (predicate instanceof PropertyMemberPredicate) {
					subLabel = ((PropertyMemberPredicate) predicate)
							.getFirstTerm().getLabel();
					objLabel = ((PropertyMemberPredicate) predicate)
							.getSecondTerm().getLabel();
					relLabel = ((PropertyMemberPredicate) predicate)
							.getProperty().getLabel();
					label = label + truncateUri(relLabel) + "("
							+ truncateUri(subLabel) + ","
							+ truncateUri(objLabel) + ")";
				} else if (predicate instanceof ConceptMemberPredicate) {
					subLabel = ((ConceptMemberPredicate) predicate).getTerm()
							.getLabel();
					objLabel = ((ConceptMemberPredicate) predicate)
							.getConcept().getLabel();
					relLabel = "type";
					label = label + truncateUri(relLabel) + "("
							+ truncateUri(subLabel) + ","
		...
		initialIds.add("1");
		CheckboxDialog dialog = new CheckboxDialog(m_viewers.iterator().next()
				.getControl().getShell(), "Results of Interpretation",
				"Selection options", "Select one of the options", ids, labels,
				initialIds, new CheckboxDialog.CheckboxValidator() {
					public String validate(Collection options) {
						return (options.size() != 1) ? "Select one option"
								: null;
					}
				});
	...
	/**
	 * Returns the top k queries in sparql, if available.
	 * 
	 * @param k
	 * @return
	 */
	public Vector<QueryWrapper> getTopKQueries(int k) {

		Vector<QueryWrapper> sparqlQueries = new Vector<QueryWrapper>();
		...
		if (queries != null) {
			...
			for (int i = 0; i < k && iter.hasNext(); i++) {
				selectedQuery = iter.next();

				sparqlQueries.addElement(m_translator.translate2SparqlQuery(
						selectedQuery, m_interpretator
								.getRankedVariables(selectedQuery)));
	...
	public boolean updateIntepretationCommandlineVersion() {
		...
		if (queries != null) {
			...
			m_modeldefinition = m_translator.translate2ModelDefinition(
					selectedQuery, m_modeldefinition);
			return true;
	...
	public void updateIntepretation() {
		...
		if (queries != null) {
			...
			// TODO: popup a menu for the user to choose the definition instead
			// of the highest ranked one
			if (queries.size() > 1) {
			...
			if (selected == true) {
				// TODO instead of overwriting --> do a merge of the returned
				// model definition with the current model definition
				m_modeldefinition = m_translator.translate2ModelDefinition(
						selectedQuery, m_modeldefinition);
			}
		} else {
			...
			MessageBox mb = new MessageBox(m_viewers.iterator().next()
					.getControl().getShell(), SWT.ICON_WORKING | SWT.OK);
			mb.setText(title);
	...
	public void updateResult() {
		...
		QueryWrapper sparqlQuery = m_translator.translate2SparqlQuery(
				selectedQuery, m_interpretator
						.getRankedVariables(selectedQuery));
	...
	public void refreshViewers(ITreeNode node) {
		for (Viewer viewer : m_viewers) {
			viewer.refresh();
			if (viewer instanceof TreeViewer) {
				((TreeViewer) viewer).expandToLevel(node,
	...
	public IQuery getQuery() {
		if (selectedQuery != null)
			...
			selectedQuery = new LinkedHashSet<OWLPredicate>();
		m_modeldefinition.getSelectedQuery(m_modeldefinition, selectedQuery);
		Set<String[]> triples = m_translator.translate2StringTriples(
				selectedQuery, m_interpretator
						.getRankedVariables(selectedQuery));
