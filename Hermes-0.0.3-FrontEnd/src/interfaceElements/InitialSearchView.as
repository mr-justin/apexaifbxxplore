package interfaceElements
{
	...
	public class InitialSearchView
	{
		...
		private function drawGraph(index:int) : void {
			...
			//Alert.show(ObjectUtil.toString(g));
			this.graphDrawingZone.removeAllChildren();
			this.graphDrawingZone.addChild(this.graph);
			this.variableList.removeAll();
			this.variableNameList.removeAll();
			...
			if(g.vertexList.length > 0) {
				// List of the variable letters
				var vList:ArrayCollection = new ArrayCollection;
				// List of the URIs
				var uList:ArrayCollection = new ArrayCollection;
				// Fill these lists
				for(var j:int = 0; j < g.vertexList.length; j++) {
					// Get the vertex
					var vertex:Facet = Facet(g.vertexList.getItemAt(j));
					// If this is a concept
					if(vertex is Concept) {
						vList.addItem(Concept(vertex).variableLetter);
						uList.addItem(vertex.URI);
					} else {
						vList.addItem("");
						uList.addItem("");
					}
				...
				var rootNodeURI:String = Facet(g.vertexList.getItemAt(0)).URI;
				...
				if(g.vertexList.getItemAt(0) is Concept) {
					...
					// Add the URI part if available for all the mapped instances
					rootNodeStr += "!";
					for(var k:int = 0; k < vList.length; k++) {
						if(vList.getItemAt(k) == letter)
							rootNodeStr += uList.getItemAt(k);
					}
				} else {
		...
		private function fetcher(fetchHandler:Function, uri:String, index:int,pageSize:int) : void {
			...
			var uList:ArrayCollection = new ArrayCollection;
			// Fill these lists
			for(var j:int = 0; j < this.currentQueryGraph.vertexList.length; j++) {
				// Get the vertex
				var vertex:Facet = Facet(this.currentQueryGraph.vertexList.getItemAt(j));
				// If this is a concept
				if(vertex is Concept) {
					vList.addItem(Concept(vertex).variableLetter);
					uList.addItem(vertex.URI);
			...
			for(var i:int = 0; i < this.currentQueryGraph.edgeList.length; i++) {
				...
				if(e.fromElement is Concept) {
					...
					fromElementStr += "!";
					for(var k:int = 0; k < vList.length; k++) {
						if(String(vList.getItemAt(k)) == letter)
							fromElementStr += uList.getItemAt(k);
					}
				} else {
				...
				if(e.toElement is Concept) {		
					...
					// Add the URI part if available for all the mapped instances
					toElementStr += "!";
					for(var l:int = 0; l < vList.length; l++) {
						if(vList.getItemAt(l) == letter2)
							toElementStr += uList.getItemAt(l);
					}
				} else {
