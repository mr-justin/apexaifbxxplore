package interfaceElements
{
	...
	public class SearchHistoryView
	{
		...
		public function addKeywordFacet(text:String, sourceName:String) : void {
			...
			if(this.elementTypeList.length > 2 && (this.elementTypeList.getItemAt(this.elementTypeList.length - 2) == "relation"
			   || this.elementTypeList.getItemAt(this.elementTypeList.length - 2) == "relationSuggestion"))
				addGenericFacet(text, sourceName, "#88ff7f", "#0cff00", true);
			else
				addGenericFacet(text, sourceName, "#88ff7f", "#0cff00", false);	
		}
		...
		public function addConceptFacet(text:String, sourceName:String) : void {
			...
			if(this.elementTypeList.length > 2 && (this.elementTypeList.getItemAt(this.elementTypeList.length - 2) == "relation"
			   || this.elementTypeList.getItemAt(this.elementTypeList.length - 2) == "relationSuggestion"))
				addGenericFacet(text, sourceName, "#77aaff", "#3b85ff", true);
			else			
				addGenericFacet(text, sourceName, "#77aaff", "#3b85ff", false);
		}
		...
		public function addRelationFacet(text:String, sourceName:String) : void {
			...
			addGenericFacet(text, sourceName, "#ffc06e", "#ff7e00", true);
		}
		 ...
		 * @param text The text to display on the button
		 * @param sourceName The name of the associated source
		 * @param color1 A string representing the hexadecimal value of the color
		 * @param color2 A string representing the hexadecimal value of the color
		 * @param arrow Set to true if an arrow should be placed
		 */
		private function addGenericFacet(text:String, sourceName:String, color1:String, color2:String, arrow:Boolean) : void {
			// 1. Create the button
			...
			btn.setStyle("fillColors", [color1, color2]);
