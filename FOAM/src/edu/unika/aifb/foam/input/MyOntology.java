...
public class MyOntology implements Structure{
	...
	private static final String[] ONTOLOGYFILES = {"C:/Work/Petri/PNOntology.owl","C:/Work/Petri/travelBooking.owl"};
...
//				resolver.registerReplacement(logicalURIs[i],physicalURIs[i]);
			...
			OntologyManager connection = KAON2Manager.newOntologyManager();
			connection.setOntologyResolver(resolver);
			Map parameters = Collections.emptyMap();
			for (int i = 0; i<physicalURIs.length; i++) {
				ontologies[i] = connection.openOntology(logicalURIs[i], parameters);
			}
