 ...
 * </copyright>
 *
 * $Id: TermFactoryImpl.java,v 1.2 2007/04/18 06:55:34 lql Exp $
 */
...
public class TermFactoryImpl implements TermFactory {
	...
	public Term createTermForInstances(GeneralCategory cat) {
		if( cat instanceof Category )
			return createTerm( ((Category)cat).getIDofURI(), FieldType.INSTANCE_TYPE_OF );
        ...
        if (cat instanceof EnumerationCategory) {
            ...
            for (int i=0; i<instances.length; i++) {
                term.addTerm(this.createTerm(instances[i].getIDofURI(), FieldType.ID));                
            }
            return term;
        }
	...
	public Term createTermForSubCategories(Category cat) {
		return createTerm( cat.getIDofURI(), FieldType.CATEGORY_SUB_OF );
	}
	...
	public Term createTermForSubRelations(Relation rel) {
		return createTerm( rel.getIDofURI(), FieldType.RELATION_SUB_OF );
	}
	...
	public Term createTermForSubjects(Relation rel) {
		return createTerm( rel.getIDofURI(), FieldType.INSTANCE_SUBJECT_OF );
	}
	...
	public Term createTermForObjects(Relation rel) {
		return createTerm( rel.getIDofURI(), FieldType.INSTANCE_OBJECT_OF );
	}

    public Term createTermForSuperCategories(Category cat)
    {
        return createTerm( cat.getIDofURI(), FieldType.CATEGORY_SUPER_OF );
    }

    public Term createTermForSuperRelations(Relation rel)
    {
        return createTerm( rel.getIDofURI(), FieldType.RELATION_SUPER_OF );
    }
}
