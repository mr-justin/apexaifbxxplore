 ...
 * </copyright>
 *
 * $Id: IndexReaderImpl.java,v 1.2 2007/04/18 06:55:34 lql Exp $
 */
...
public class IndexReaderImpl implements IndexReader {
	...
	protected IndexReaderImpl(String indexPath, IndexType type) throws IOException {
    ...
    private Query toLuceneQuery(Term term){
		...
		if(term instanceof CompoundTerm){
			...
			if (term.getFieldType()==FieldType.TEXT) {
				if (term.getString().contains(" ")) {
					String[] split = term.getString().split(" ");
					  PhraseQuery query = new PhraseQuery();
					  for (int i=0; i<split.length; i++) {
					    query.add(new org.apache.lucene.index.Term(FieldType.TEXT.toString(), split[i]));
					  }
