...
final class SegmentTermPositions extends SegmentTermDocs implements
		TermPositions, DocPositionStream {
	...
	SegmentTermPositions(SegmentReader p) {
		super(p);
		this.proxStream = null; // the proxStream will be cloned lazily when
	...
	public byte[] getPayload(byte[] data, int offset) throws IOException {
		if (!needToLoadPayload) {
			throw new IOException(
					"Payload cannot be loaded more than once for the same term position.");
		}
