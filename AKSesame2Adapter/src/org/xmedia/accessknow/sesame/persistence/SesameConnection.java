...
//		TODO 
	...
	public IOntology loadOntology(Map<String, Object> parameters) throws MissingParameterException, InvalidParameterException, OntologyLoadException {
		...
		IOntology theOntology = SesameOntology.loadOntology(
				ontologyUri, 
				getRemoteHandle(ontologyUri),
				this, 
				getReificationSwitch(parameters));
		
		addOntology(theOntology);
	...
	private void deleteLocalOntology(String ontologyUri) throws Exception {
		...
		// Close the ontology
		try {
			closeOntology(loadOntology(parameters));
		} catch (MissingParameterException e) {
		} catch (InvalidParameterException e) {
		} catch (OntologyLoadException e) {
			// Do nothing beacuse we it _may_ be a memory ontology.
		}
		...
		File dataDir = getSesameRepositoryFactory().getRepositoryDir(theOntologyUri);
		if (dataDir.exists()) {
			try {
	...
	protected SesameRepositoryFactory getSesameRepositoryFactory() {
